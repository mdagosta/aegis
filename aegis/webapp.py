...
class AegisHandler(tornado.web.RequestHandler):
    ...
    def _handle_request_exception(self, ex):
        ...
        header = "`[%s ENV   %s   %s   uid: %s   mid: %s   ip: %s]`" % (config.get_env().upper(), self.request.uri, self.tmpl['request_name'], self.get_user_id() or '-', self.get_member_id() or '-', self.request.headers.get("X-Forwarded-For", '').split(',')[0])
